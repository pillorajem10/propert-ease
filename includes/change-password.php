...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (empty($currentPasswordErr) && empty($newPasswordErr) && empty($confirmPasswordErr)) {
        ...
        if (isset($_SESSION['tenant_id'])) {
            ...
            $decryptedPassword = decryptData($user['tenant_pass'], $encryptionKey);
            ...
            if ($currentPassword == $decryptedPassword || password_verify($currentPassword, $decryptedPassword)) {
                ...
                $encryptedPrevPasswords = $user['tenant_prevpass'];
                ...
                $updateStmt = $pdo->prepare("UPDATE tenant_tbl SET tenant_pass = ?, tenant_prevpass = ? WHERE tenant_id = ?");
