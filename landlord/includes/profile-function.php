...
$landlordStmt = $pdo_01->prepare("SELECT * FROM landlord_tbl WHERE landlord_id = ?");
...
$profile = decryptedData($landlord['landlord_dp'], $landlord['encryption_key']);
$fname = decryptedData($landlord['landlord_fname'], $landlord['encryption_key']);
$lname = decryptedData($landlord['landlord_lname'], $landlord['encryption_key']);
$temail = $landlord['landlord_email'];
$contact = $landlord['landlord_contact'];
$taddress = decryptedData($landlord['landlord_address'], $landlord['encryption_key']);
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($firstNameErr) && empty($lastNameErr) && empty($addressErr) && empty($emailErr) && empty($profilePictureErr)) {
        ...
        $encrypted_fname = encryptData($_POST['edit-fname'], $landlord['encryption_key']);
        $encrypted_lname = encryptData($_POST['edit-lname'], $landlord['encryption_key']);
        $encrypted_address = encryptData($_POST['edit-address'], $landlord['encryption_key']);
        $updated_contact = $_POST['edit-contact'];
        $updated_email = $_POST['edit-email'];
        $encryptedProfilePicture = encryptData($_FILES['edit-profile-image']['name'], $landlord['encryption_key']);
        ...
        if (isset($_FILES["edit-profile-image"]) && $_FILES["edit-profile-image"]["error"] === UPLOAD_ERR_OK) {
            ...
            if (move_uploaded_file($_FILES["edit-profile-image"]["tmp_name"], $targetFile)) {
                // Update landlord information in the database with image path and encrypted data
                $updateStmt = $pdo_01->prepare("UPDATE landlord_tbl SET landlord_fname = ?, landlord_lname = ?, landlord_email = ?, landlord_contact = ?, landlord_address = ?, landlord_dp = ? WHERE landlord_id = ?");
                if($updateStmt->execute([$encrypted_fname, $encrypted_lname, $updated_email, $updated_contact, $encrypted_address, $encryptedProfilePicture, $_SESSION['landlord_id']])) {
            ...
            // Update landlord information in the database without changing the image path
            $updateStmt = $pdo_01->prepare("UPDATE landlord_tbl SET landlord_fname = ?, landlord_lname = ?, landlord_email = ?, landlord_contact = ?, landlord_address = ?, landlord_dp = ? WHERE landlord_id = ?");
            if($updateStmt->execute([$encrypted_fname, $encrypted_lname, $updated_email, $updated_contact, $encrypted_address,  $_SESSION['landlord_id']])) {
