...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($emailErr)) {
        try {
            ...
            $stmt = $pdo->prepare("SELECT * FROM resetpass_tbl WHERE email = ?");
            $stmt->execute([$email]);
            $existingVerification = $stmt->fetch();

                if ($existingVerification) {
                    // Check if the verification code has been used
                    if ($existingVerification['used'] == 0) {
                        // Email verification exists but not used yet, return error response
                        $response = array('error' => 'Email verification already exists!');
                        header('Content-Type: application/json');
                        echo json_encode($response);
                        exit;
                    } else {
                        // Delete existing verification entry as it has been used
                        $stmt = $pdo->prepare("DELETE FROM resetpass_tbl WHERE email = ?");
                        $stmt->execute([$email]);
                    }
                }
                ...
                $stmt = $pdo->prepare("INSERT INTO resetpass_tbl (email, code, encryption_key) VALUES (?, ?, ?)");
